...
std::queue<pcl::PointCloud<pcl::PointXYZI>::Ptr> his_cloud_queue;
// 定义里程计队列
...
pcl::PointCloud<pcl::PointXYZI> createSubmap() 
{
    ...
    if (subnum <= frames_to_process)
    {
            ...
            if (size_map<his_subnum)
            {
                ...
                {
                    *submap += *maps[size_map-1-j];
                }
            }else{
                for (size_t j = 0; j < his_subnum; j++)
                {
                    *submap += *maps[size_map-1-j];
                }
            }
