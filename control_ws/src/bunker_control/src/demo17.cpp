...
double linear_x = 0;
double angular_z = 0;
static double limit_linear_normal = 0.5;
static double limit_linear_narrow = 0.3;
static double limit_linear_gate = 0.3;
static double limit_linear_obs_avoid = 0.3;
...
int main(int argc, char** argv) {
    ...
    while (ros::ok()) {
        ...
        if(planning==true && work_style==0)
        {
            if(linear_x> limit_linear_obs_avoid) linear_x= limit_linear_obs_avoid;
            if(linear_x<(limit_linear_obs_avoid * -1)) linear_x=-1 * limit_linear_obs_avoid;
        }
        else if(work_style==1)//狭窄
        {
            if(linear_x>limit_linear_narrow) linear_x=limit_linear_narrow;
            if(linear_x<limit_linear_narrow * -1) linear_x=limit_linear_narrow * -1;
        }
        else if(work_style==2)//过门
        {
            if(linear_x>limit_linear_gate) linear_x=limit_linear_gate;
            if(linear_x<limit_linear_gate * -1) linear_x=limit_linear_gate * -1;
        }
        else
        {
            if(linear_x>limit_linear_normal) linear_x=limit_linear_normal;
            if(linear_x<limit_linear_normal * -1) linear_x=limit_linear_normal * -1;
        }
