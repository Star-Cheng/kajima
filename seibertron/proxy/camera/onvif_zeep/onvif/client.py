...
from camera.onvif_zeep.onvif.exceptions import ONVIFError
from camera.onvif_zeep.onvif.definition import SERVICES
...
class UsernameDigestTokenDtDiff(UsernameToken):
    """
    UsernameDigestToken class, with a time_utils offset parameter that can be adjusted;
    This allows authentication on cameras without being time_utils synchronized.
    ...
    def __init__(self, user, passw, dt_diff=None, **kwargs):
        ...
        self.dt_diff = dt_diff  # Date/time_utils difference in datetime.timedelta
...
class ONVIFCamera(object):
    ...
    adjust_time parameter allows authentication on cameras without being time_utils synchronized.
    Please note that using NTP on both end is the recommended solution,
    this should only be used in "safe" environments.
